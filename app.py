...
st.markdown("""
    ...
    /* Restyle BOTH standard action buttons and native link buttons to solid Black & Gold */
            ...
            for index, watch in catalog_df.iterrows():
                with st.container():
                    ...
                    if st.button("Place Order", key=f"order_btn_{index}", use_container_width=True):
                        ...
                        # Secure direct message communication link string
                        whatsapp_href = f"https://wa.me{encoded_payload}"
                        
                        # 🔥 THE ANTI-BLOCK FIX:
                        # Uses native link_button which browsers naturally white-list and trust completely
                        st.link_button("💬 Open WhatsApp & Message Store Owner", url=whatsapp_href, use_container_width=True)
                        st.markdown("---")
