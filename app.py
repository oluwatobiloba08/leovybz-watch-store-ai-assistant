            ...
            for index, watch in catalog_df.iterrows():
                with st.container():
                    ...
                    if st.button("Place Order", key=f"order_btn_{index}", use_container_width=True):
                        ...
                        # Direct DM routing API link
                        whatsapp_href = f"https://wa.me{encoded_payload}"
                        
                        # 🔥 SECURE MARKDOWN LINK:
                        # Bypasses browser pop-up blocks perfectly by using trusted standard markdown strings
                        st.markdown(f"### 🟢 [➡️ CLICK HERE TO OPEN WHATSAPP DM & PLACE ORDER]({whatsapp_href})")
                        st.markdown("---")
